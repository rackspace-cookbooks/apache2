...
define :apache_module, :enable => true, :conf => false do
  ...
  params[:module_path] = params[:module_path] || "#{node['rackspace_apache']['libexecdir']}/#{params[:filename]}"
  ...
  if params[:enable]
    ...
    execute "a2dismod #{params[:name]}" do
      ...
      notifies :restart, 'service[apache2]'
      only_if { ::File.symlink?("#{node['rackspace_apache']['dir']}/mods-enabled/#{params[:name]}.load") }
    end
  end
end
